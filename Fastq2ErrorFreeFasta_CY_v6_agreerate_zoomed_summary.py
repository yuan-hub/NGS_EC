...
# - Calculate summary cluster agreement rate metrics (per cluster with mismatch) 
    ...
    for j in range(0,len(readlist[0])):
      ...
      for b in bases.keys():
        ...
        if pos_agree_rate > pos_agree_rate_max:
          ...
          bs = b
      realread += bs	# Appends consensus base
      if pos_agree_rate_max < 0.9:	#! This threshold needs to be updated based on range of interest
        return ({}, 0)	# Returns nothing if max. agreement rate falls below range of interest 
      else:
        if len(bases.keys()) > 1:
          results[j] = [len(bases.keys()),pos_agree_rate_max]	# Saves num of bases and max agreement rate
    # If consensus is established through majority voting & # of mismatches > 0
    if len(realread) == len(readlist[0]) and len(results.keys()) > 0:
      for read in readlist:
        if read != realread: mismatches += 1	# Calculate num of mismatches
      # Check whether max positional agreement rates fall within range of interest
      if min(i[1] for i in results.values()) >= 0.9 and min(i[1] for i in results.values()) <= 1.0:	#! This threshold needs to be updated based on range of interest
        avg_rate = sum(i[1] for i in results.values())/len(results.keys())	# Calculate average max. agreement rate across all mismatched positions
        summary = {'pos':len(results.keys()), 'avg_rate':avg_rate}
...
def errorcorrect(tmp,ofile):
  ...
  ratesfile = ofile + "_summary.txt"
  ratesfile = open(ratesfile, 'w')
  ratesfile.write('Cluster_ID' + '\t' + 'Cluster_size' + '\t' + 'Num_mismatch_reads' + '\t' + 'Num_mismatch_pos' + '\t' + 'Avg_agreement_rate' + '\n')
  for tmpfile in tmpfiles:
    ...
    for ID in Rclusters.keys():
      (summary, mismatches) = EFread(Rclusters[ID])
      if mismatches != 0:
        ratesfile.write(ID + '\t' + str(len(Rclusters[ID])) + '\t' + str(mismatches) + '\t' + str(summary['pos']) + '\t' + format(summary['avg_rate'],'.2f') + '\n')
