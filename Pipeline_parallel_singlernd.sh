...
#
### Generate consensus & corresponding raw datasets for each desired coverage level
#
proc=0
echo "Starting consensus & raw dataset generation..."
if [ ! -d $Output_dir/Reads_WIP/fastq ]; then
	mkdir $Output_dir/Reads_WIP/fastq
	cd $Output_dir/Reads_WIP/fastq
fi
for cvg in "${coverages[@]}"; do
	echo "Generating datasets for coverage level $cvg"
	python $Proj_dir/Software/Scripts/Fastq2ErrorFreeFasta_CY_v7_singlernd.py -i $Data_dir/Reads_Raw/full/ -o $Output_dir/Reads_WIP/fastq/Reads -b 1-3 -p 4-12 -F _R1 -R _R2 -d 4 -w $num_raw -c $cvg &
	while [ ! -d tmp$(($proc+2)) ]; do
		#echo "Waiting..."
		sleep 1
	done
	((proc++))
...
wait
# #
# ### Convert consensus & raw output from Fasta to Fastq
# #
# echo "Starting fasta to fastq conversion..."
# if [ ! -d $Output_dir/Reads_WIP/fastq ]; then
	# mkdir $Output_dir/Reads_WIP/fastq
# fi
# cd $Output_dir/Reads_WIP/fasta
# for file in *.fasta; do
	# echo "Converting fasta file: $file"
	# filename="${file%.*}"	# Removing file extensions
	# python $Proj_dir/Software/Scripts/Fasta2Fastq.py ../fastq/$filename"_R1".fastq ../fastq/$filename"_R2".fastq < $file
# done
# rm *	# Remove fasta files after conversion
#
### Move WIP read files to correct folders
#
cd $Output_dir/Reads_WIP/fastq/
if [ ! -d $Output_dir/Reads_Consensus ]; then
	mkdir $Output_dir/Reads_Consensus
fi
if [ ! -d $Output_dir/Reads_Raw ]; then
	mkdir $Output_dir/Reads_Raw
fi
mv Reads_*consensus*.fastq $Output_dir/Reads_Consensus/
mv Reads_*raw*.fastq $Output_dir/Reads_Raw/
# Create interleaved read files
echo "Creating interleaved consensus files..."
cd $Output_dir/Reads_Consensus/
for cvg in "${coverages[@]}"; do
	$seqtk mergepe "Reads_c"$cvg"_consensus_R1.fastq" "Reads_c"$cvg"_consensus_R2.fastq" > "Reads_c"$cvg"_consensus_merged.fastq"
done
echo "Creating interleaved raw files..."
cd $Output_dir/Reads_Raw/
for cvg in "${coverages[@]}"; do
	for((i=1;i<=$num_raw;i++)); do
		$seqtk mergepe "Reads_c"$cvg"_raw"$i"_R1.fastq" "Reads_c"$cvg"_raw"$i"_R2.fastq" > "Reads_c"$cvg"_raw"$i"_merged.fastq"
	done
done
#
