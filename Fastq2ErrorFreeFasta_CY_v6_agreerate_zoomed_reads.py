...
# - Output all reads within clusters with mismatched reads
#   for a specific range of minimum cluster consensus rates (e.g. from 0.74 - 8)
#
#####################################################################
#############################   ENJOY   #############################
#####################################################################
...
def EFread(readlist):
  ...
  pos_agree_rate_max = []
  results = {'success':0, 'realread':'', 'cls_agree_rate_min':0}
  for j in range(0,len(readlist[0])):
    ...
    if totalread >= 2:
      ...
      for b in bases.keys():
        ...
        if pos_agree_rate > pos_agree_rate_max[j]:
          pos_agree_rate_max[j] = pos_agree_rate
        if pos_agree_rate > 0.9:
          bs += b
      if len(bs) == 1:
        results['realread'] += bs
  if len(results['realread']) == len(readlist[0]): 
    results['success'] = 1

  results['cls_agree_rate_min'] = min(pos_agree_rate_max)
  return results
...
def errorcorrect(tmp,ofile):
  ...
  ratesfile = ofile + "_agreerates.txt"
  ...
  for tmpfile in tmpfiles:
    ...
    for ID in Rclusters.keys():
      ...
      if efreeread['cls_agree_rate_min'] >= 0.74 and efreeread['cls_agree_rate_min'] <= 0.8:	#! This threshold needs to be updated based on range of interest
        ratesfile.write('ID:' + ID + '\t' + str(len(Rclusters[ID])) + '\t' + format(efreeread['cls_agree_rate_min'],'.2f') + '\n')
        for read in Rclusters[ID]:
          ratesfile.write(read + '\n')
        ratesfile.write('\n\n')
      #print "Processed ", count, " of ", total_clusters, " clusters."
      count += 1
